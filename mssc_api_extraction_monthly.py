...
def check_total_count(dates,file_paths):
    ...
    if total_counts != 0:
        ...
        generate_all_files(dates,file_paths)
    else:
        print("No records today!")

#This function calls all other functions        
def generate_all_files(curr_date,file_paths):
...
def dates_generation(file_paths):
    from datetime import date, timedelta
    
    file_paths = file_paths
    
    start_dt = date(2024,6, 1)
    end_dt = date(2024, 6, 17)
    ...
    #calling call_check_count function
    call_check_count(dates,file_paths)

def call_check_count(dates,file_paths):
    file_paths = file_paths
    for i in range(len(dates)):
        check_total_count(dates[i],file_paths)
